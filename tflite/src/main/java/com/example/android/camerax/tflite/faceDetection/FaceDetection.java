...
public class FaceDetection {
    ...
    public float[][][] decodeBoxes(float[][][] raw_boxes) {
        ...
        float[][][] boxes = new float[raw_boxes.length][raw_boxes[0].length][num_points*2];
        for(int i = 0; i < raw_boxes.length; i++) {
            for(int j = 0; j < raw_boxes[0].length; j++) {
                for(int k = 0; k < num_points*2; k++) {
                    boxes[i][j][k] = raw_boxes[i][j][k]/scale;
                }
            }
        }
        // adjust center coordinates and key points to anchor positions
        for(int i = 0; i < boxes.length; i++) {
            boxes[i][0][0] += this.anchors[0];
            boxes[i][0][1] += this.anchors[1];
        }
        for(int i = 2; i < num_points; i++) {
            for(int j = 0; j < boxes.length; j++) {
                boxes[j][i][0] += this.anchors[0];
                boxes[j][i][1] += this.anchors[1];
            }
        }
